...
Node* createNode(int v) {
    Node* newNode = malloc(sizeof(Node)); newNode->vertex = v;
...
void enqueue(Queue* q, int value) {
    if (q->rear == MAX_VERTICES - 1) { printf("Queue is full\n"); return; }
...
int main() {
    Graph graph; int vertices = 6; initGraph(&graph, vertices);
    addEdge(&graph, 0, 1); addEdge(&graph, 0, 2); addEdge(&graph, 1, 3);
    addEdge(&graph, 1, 4); addEdge(&graph, 2, 4); addEdge(&graph, 3, 4);
