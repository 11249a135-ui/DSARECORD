...
void enqueue(Queue* q, Node* node) { if (q->rear < MAX_QUEUE - 1) q->items[++q->rear] = node; }
Node* dequeue(Queue* q) { if (!isEmpty(q)) return q->items[q->front++]; return NULL; }
void levelOrder(Node* root) {
    if (root == NULL) return;
    Queue q; initQueue(&q); enqueue(&q, root);
    while (!isEmpty(&q)) {
        Node* current = dequeue(&q); printf("%d ", current->data);
